...
async function getSale(id) {
  const conn = await connect.connect();
  try {
    let query = await conn.query("SELECT * FROM sales WHERE sale_id = $1", [
      id,
    ]);
    return query.rows[0];
  } catch (err) {
...
async function deleteSale(id) {
  const conn = await connect.connect();
  try {
    let query = await conn.query("DELETE FROM sales WHERE sale_id = $1", [id]);
  } catch (err) {
    throw err;
  } finally {
    conn.release();
...
async function updateSale(sale) {
  ...
  try {
    let query = await conn.query(
      ...
      [sale.value, sale.date, sale.client_id, sale.sale_id]
    );
    return query.rows[0];
  } catch (err) {
    throw err;
